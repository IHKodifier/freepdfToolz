...
import 'package:freepdftoolz_frontend/pages/process_page.dart';
...
void main() {
  group('ProcessPage Dedicated Route Tests', () {
    ...
    testWidgets(
      'ProcessPage renders AppHeader, OcrProgressView hero card, and AdSenseBanner underneath',
    ...
    testWidgets(
      'Route navigation /process/job_123 resolves properly from MaterialApp',
      (WidgetTester tester) async {
        ...
        Navigator.pushNamed(
          ...
          arguments: {'filename': 'scanned_invoice.pdf', 'fileSize': 2048576},
        );
        await tester.pump();
        ...
        expect(
          find.descendant(
            of: find.byType(ProcessPage),
            matching: find.byType(AdSenseBanner),
          ),
          findsOneWidget,
        );
      },
    ...
    testWidgets('ProcessPage reset button navigates back to landing page', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(const FreeOcrApp());
      await tester.pump();
    ...
    testWidgets(
      'ProcessPage with in-flight upload mounts cleanly with AdSenseBanner',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: ProcessPage(
    ...
    testWidgets('OcrProgressView renders UPLOADING status with byte metrics', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        const MaterialApp(
